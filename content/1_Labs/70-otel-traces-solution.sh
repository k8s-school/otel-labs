...
for i in $(seq 1 24); do
    TRACES=$(curl -sSf "http://localhost:8080/jaeger/ui/api/traces?service=review-service&operation=POST%20%2Fapi%2Freviews&limit=20" || true)
...
found=""
for i in $(seq 1 24); do
    if curl -sSf "http://localhost:8080/jaeger/ui/api/traces?service=review-service&tags=%7B%22error%22%3A%22true%22%7D&limit=10" \
            | grep -q "traceID"; then
        found=yes
        break
    fi
