...
# and check the datasources are wired. Also checks what lab 4.2 reads: the
...
# --- lab 4.2 reads a dashboard shipped by the demo: check it is in place ---
...
# pins), so the day the chart is unpinned lab 4.2 would silently go stale.
...
if [ "$found" != true ]; then
    ...
    echo "       lab 4.2 (exemplars) documents that metric name - check the chart version"
    exit 1
fi
...
if grep -q '"traceID"' <<< "$EX" || grep -q '"trace_id"' <<< "$EX"; then
    ...
    echo "WARNING: no exemplar returned yet - lab 4.2 needs traffic on the cart service"
fi

echo "Lab 4 OK: dashboard in Grafana, lab 4.2 dashboard in place"
