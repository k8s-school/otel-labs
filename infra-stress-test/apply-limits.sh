...
apply() {   # apply <deploy> <request> <limit>
    for u in $USERS; do
        printf '%-10s %-12s ' "$u" "$1"
        timeout 60 sudo -u "$u" -i kubectl -n otel-demo set resources deploy "$1" --requests=memory="$2" --limits=memory="$3" 2>&1 | tail -1 | cut -c1-70
    done
}
if [ $# -eq 3 ]; then apply "$1" "$2" "$3"; else
